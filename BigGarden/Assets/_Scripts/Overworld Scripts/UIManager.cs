...
public class UIManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (VD.isActive) //If there is a dialogue active
        {
            ...
            {
                if (Input.GetKeyDown(player.downKey))
                {
                    if (data.commentIndex < currentChoices.Count - 1)
                        data.commentIndex++;
                }
                if (Input.GetKeyDown(player.upKey))
                {
    ...
    public void SetOptions(string[] choices)
    {
        ...
        for (int i = 0; i < choices.Length; i++)
        {
            ...
            newOp.GetComponent<RectTransform>().anchoredPosition = new Vector2(42.23f, -20 - (20 * i));
